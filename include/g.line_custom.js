 ...
 ** 
 \*/
            symbols = paper.set(),
            line;
		coordx = [], coordy = [], coordXX = [], coordYY = [];//ADDED BY SUPREET		

		var k =0; // ADDED BY SUPREET
        for (i = 0, ii = valuesy.length; i < ii; i++) {
            if (!opts.nostroke) {
			...
			//document.write(X+","+Y+"\n");
				...
				coordx[j] = X.toFixed(2)*1; //ADDED BY SUPREET		
				coordy[j] = Y.toFixed(2)*1; //ADDED BY SUPREET	
            }
        ...
        function createColumns(f) {
            ...
            Xs = Xs2.sort(function(a,b){return a - b}); //Fix added by Supreet
            ii = Xs.length;
